...
model_paths = {
    'mbv2_voc': './data/weights/mbv2_voc_rflw.ckpt',
               }
...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, width_mult=1., default_rate=1):
        ...
        self.n_layers = len(self.interverted_residual_setting)
        for idx, (t, c, n, s) in enumerate(self.interverted_residual_setting):
...
def mbv2(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load(model_paths['mbv2_{}'.format(str(pretrained))]), strict=False)
    return model
